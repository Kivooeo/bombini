...
fn test_6_2_procmon_bprm_check_parent_binary() {
    // `xargs -a /dev/null /bin/bash` makes xargs fork a child that execs
    // `/bin/bash`. At the bprm_check for that `bash` exec, the exec'ing process
    // is the forked child (still named `xargs`) whose direct parent is the
    // `xargs` process itself. The parent_binary_name scope matches via the
    // parent lookup path (not the current-binary path), selecting that event.
    let config_contents = r#"
bprm_check:
  enabled: true
  rules:
...
"#;
    ...
    print_example_events!(&events);
    assert_eq!(events.matches("\"type\":\"BprmCheck\"").count(), 1);
    assert_eq!(events.matches("\"rule\":\"ParentBinaryTestRule\"").count(), 1);
