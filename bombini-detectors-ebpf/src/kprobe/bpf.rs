...
impl ProbeFn {
    ...
    unsafe fn get_depth(&self) -> Option<u8> {
        if let Some(d) = unsafe { FN_DEPTH.get(&self.depth_key()) } {
            return Some(*d);
        }
        None
    }
    ...
    pub unsafe fn save_ctx(&self, ctx: &ProbeContext) -> Result<(), Error> {
        let depth = unsafe { self.get_depth().unwrap_or(0).wrapping_add(1) };

        let k = unsafe { self.ctx_key(depth) };
        (unsafe {
            SAVED_CTX
        ...
        unsafe { self.update_depth(depth) }
    }

    #[inline(always)]
    pub unsafe fn restore_ctx(&self) -> Result<&'static mut KProbeEntryContext, Error> {
        let depth = unsafe { self.get_depth().ok_or(Error::DepthGet) }?;
        let k = unsafe { self.ctx_key(depth) };
        let ctx = unsafe { SAVED_CTX.get_ptr_mut(&k).ok_or(Error::CtxGet) }?;
        unsafe { Ok(&mut *ctx) }
    }
    ...
    pub unsafe fn clean_ctx(&self) -> Result<(), Error> {
        ...
        let _ = unsafe { SAVED_CTX.remove(&self.ctx_key(depth)) };
        // we decrement depth and update map
        unsafe { self.update_depth(depth.wrapping_sub(1)) }
    }
    ...
    pub unsafe fn depth_key(&self) -> u128 {
        unsafe { core::mem::transmute([bpf_task_tracking_id(), *self as u64]) }
    }
}
