...
use super::shim::{self, *};
use super::{CoRe, core_read_kernel, file, rust_shim_kernel_impl};

#[allow(non_camel_case_types)]
pub type fdtable = CoRe<shim::fdtable>;

impl fdtable {
    rust_shim_kernel_impl!(fdtable, max_fds, u32);
    rust_shim_kernel_impl!(fdtable, fd, *mut *mut shim::file);
}

#[allow(non_camel_case_types)]
pub type files_struct = CoRe<shim::files_struct>;

impl files_struct {
    rust_shim_kernel_impl!(files_struct, fdt, fdtable);
    rust_shim_kernel_impl!(files_struct, fd_array, *mut *mut shim::file);
