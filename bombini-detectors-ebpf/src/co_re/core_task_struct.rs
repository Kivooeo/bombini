...
use super::shim::{self, *};
use super::{
    CoRe, core_read_kernel, cred, file, files_struct, mm_struct, nsproxy, rust_shim_kernel_impl,
    task_group,
};

#[allow(non_camel_case_types)]
pub type task_struct = CoRe<shim::task_struct>;
...
impl task_struct {
    ...
    pub unsafe fn current_btf() -> Self {
        Self::from_ptr(unsafe { bpf_get_current_task_btf() } as *const _)
    }
    ...
    pub unsafe fn comm_array(&self) -> Option<[u8; 16]> {
        let mut comm = [0u8; 16];
        (unsafe { bpf_probe_read_kernel_buf(self.comm()?, comm.as_mut_slice()).ok() })?;
        Some(comm)
    }
