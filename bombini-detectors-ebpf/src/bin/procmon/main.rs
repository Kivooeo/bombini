...
/// up by `ppid`). When the parent is not tracked the keys are filled from the
/// zeroed `ZERO_MAP`, so they never match a real `parent_binary_*` entry.
/// Yields `(parent_name, parent_path, parent_prefix)` scratch-key references.
...
#[map]
static PROCMON_PRCTL_PBINPREFIX_MAP: LpmTrie<PathPrefixMapKey, u8> =
    LpmTrie::with_max_entries(1, 0);
// Filter maps end
