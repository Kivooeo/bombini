...
fn fill_parent_name_key(ppid: u32) -> Result<(), i32> {
    ...
    unsafe {
        let _ = fill_name_map!(NETMON_PARENT_FILE_NAME_MAP, src);
    }
    Ok(())
}
...
fn fill_parent_path_key(ppid: u32) -> Result<(), i32> {
    ...
    unsafe {
        let _ = fill_path_map!(NETMON_PARENT_PATH_MAP, src);
    }
    Ok(())
}
...
fn fill_parent_prefix_key(ppid: u32) -> Result<(), i32> {
    ...
    unsafe {
        let _ = fill_prefix_map!(NETMON_PARENT_PATH_PREFIX_MAP, src);
    }
    Ok(())
}
...
macro_rules! fill_parent_keys {
    ($proc:expr) => {{
        ...
        let Some(parent_name) = NETMON_PARENT_FILE_NAME_MAP
            .get_ptr_mut(0)
            .and_then(|p| p.as_mut())
        else {
            return Err(-1);
        };
        let Some(parent_path) = NETMON_PARENT_PATH_MAP
            .get_ptr_mut(0)
            .and_then(|p| p.as_mut())
        else {
            return Err(-1);
        };
        let Some(parent_prefix) = NETMON_PARENT_PATH_PREFIX_MAP
            .get_ptr_mut(0)
            .and_then(|p| p.as_mut())
        else {
            return Err(-1);
...
#[map]
static NETMON_EGRESS_PBINPATH_MAP: HashMap<PathMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static NETMON_EGRESS_PBINNAME_MAP: HashMap<FileNameMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static NETMON_EGRESS_PBINPREFIX_MAP: LpmTrie<PathPrefixMapKey, u8> =
    LpmTrie::with_max_entries(1, 0);
...
#[map]
static NETMON_INGRESS_PBINPATH_MAP: HashMap<PathMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static NETMON_INGRESS_PBINNAME_MAP: HashMap<FileNameMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static NETMON_INGRESS_PBINPREFIX_MAP: LpmTrie<PathPrefixMapKey, u8> =
    LpmTrie::with_max_entries(1, 0);
// Filter maps end
