...
fn try_detect(ctx: LsmContext, generic_event: &mut GenericEvent) -> Result<i32, i32> {
    ...
    unsafe {
        ...
        if euid == 0 {
            ...
            bpf_probe_read_kernel_str_bytes(d_name_ptr, filename).map_err(|_| 0_i32)?;
                ...
                || (filename[0] == b'z' && filename[1] == b's' && filename[2] == b'h')
            {
                ...
                let parent = core_read_kernel!(task, parent).ok_or(0i32)?;
                let mut ppid = core_read_kernel!(parent, tgid).ok_or(0i32)? as u32;
